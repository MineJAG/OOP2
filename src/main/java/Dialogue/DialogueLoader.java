        ...
        try (BufferedReader bR = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((rawLine = bR.readLine()) != null) {
                ...
                if (hasChar(header.toLowerCase(), '$')) {
                    int dIndex = header.indexOf('$');
                    if (dIndex <= 0 || dIndex == header.length() - 1) {
                        throw new IllegalArgumentException("Invalid option header: \"" + header + "\" in line: " + rawLine);
                    }
                    String dialogueId = header.substring(0, dIndex);
                    String optionId = header.substring(dIndex + 1);
                    Map<String, DialogueLine> npcDialogue = dialogue.get(currentNpc);
                    DialogueLine dl = npcDialogue.get(dialogueId);
                    if (dl == null) {
                        throw new IllegalStateException(
                                "Option references non-existent dialogue ID: " + dialogueId + " in line: " + rawLine
                        );
                    }
